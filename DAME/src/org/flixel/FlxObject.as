package org.flixel
{
	...
	public class FlxObject extends FlxRect
	{
		...
		public function getScreenXY(Point:FlxPoint=null):FlxPoint
		{
			if (Point == null)
				Point = new FlxPoint();
			Point.x = FlxU.floor(x + FlxU.roundingError + FlxG.extraScroll.x)+FlxU.floor(FlxG.scroll.x*scrollFactor.x);
			Point.y = FlxU.floor(y + FlxU.roundingError + FlxG.extraScroll.y) + FlxU.floor(FlxG.scroll.y * scrollFactor.y);
		...
		{
			getScreenXY(_point);
			if((_point.x + (width >> FlxG.zoomBitShifter) < 0) || (_point.x > FlxG.width) || (_point.y + (height>>FlxG.zoomBitShifter) < 0) || (_point.y > FlxG.height))
				return false;
			return true;
		}
