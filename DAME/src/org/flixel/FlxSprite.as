	...
	*/
	public class FlxSprite extends FlxObject
	{
		...
		*/
		...
		protected var quarterBitmap:BitmapData = null;
		protected var eighthBitmap:BitmapData = null;
		...
		protected function updateAnimation():void
		{
			...
			if((_curAnim != null) && (_curAnim.delay > 0) && (_curAnim.looped || !finished) && !paused)
			{
		...
		protected function renderSprite():void
		{
			...
			FlxG.buffer.draw(drawBmp,_mtx,null,blend,null,antialiasing);
		}
		...
		override public function getScreenXY(Point:FlxPoint=null):FlxPoint
		{
			if (Point == null)
				Point = new FlxPoint();
			Point.x = FlxU.floor(x + FlxU.roundingError + FlxG.extraScroll.x) + FlxU.floor(FlxG.scroll.x*scrollFactor.x) - offset.x;
			Point.y = FlxU.floor(y + FlxU.roundingError + FlxG.extraScroll.y) + FlxU.floor(FlxG.scroll.y * scrollFactor.y) - offset.y;
		...
		protected function calcFrameCoords( pt:FlxPoint ):void
		{
		...
		protected function calcFrame():void
		{
			...
			_flashRect.x = _point.x;
			_flashRect.y = _point.y;
			_framePixels.copyPixels(_pixels,_flashRect,_flashPointZero);
		...
		protected function redrawScaledBitmaps(sourceBitmap:BitmapData):void
		{
			var quarterWidth:int = getQuarterWidth(sourceBitmap.width);
			var quarterHeight:int = getQuarterHeight(sourceBitmap.height);
			if ( !quarterBitmap || quarterBitmap.width != quarterWidth || quarterBitmap.height != quarterHeight)
			{
				quarterBitmap = new BitmapData(quarterWidth, quarterHeight, true, 0x00000000 );
				eighthBitmap = new BitmapData(getEighthWidth(sourceBitmap.width), getEighthHeight(sourceBitmap.height), true, 0x00000000);
			}
			var mat:Matrix = new Matrix;
			mat.scale(0.25, 0.25);
			quarterBitmap.fillRect(quarterBitmap.rect, 0x00000000);
			quarterBitmap.draw(sourceBitmap, mat);
			
			mat.identity();
			mat.scale(0.125, 0.125);
			eighthBitmap.fillRect(eighthBitmap.rect, 0x00000000);
			eighthBitmap.draw(sourceBitmap, mat);
		}
		
		// pass in _framePixels or other as defaultBitmap
		protected function getDrawBitmap( defaultBitmap:BitmapData ):BitmapData
		{
			var drawBmp:BitmapData;
			
			if ( FlxG.extraZoom == 0.25 )
			{
				return quarterBitmap;
			}
		...
		protected function getQuarterWidth(normalWidth:int):int
		{
			return Math.ceil(normalWidth * 0.25);
		}
		
		protected function getQuarterHeight(normalHeight:int):int
		{
			return Math.ceil(normalHeight * 0.25);
		}
		
		protected function getEighthWidth(normalWidth:int):int
		{
			return Math.ceil(normalWidth * 0.125);
		}
		
		protected function getEighthHeight(normalHeight:int):int
		{
			return Math.ceil(normalHeight * 0.125);
		}
	}
}